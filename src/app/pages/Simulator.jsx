...
/**
* P-level sequence configuration chart.
* X-axis: Hour 0 (current, non-configurable, from patient.deviceLevel)
*         followed by Hour 1 … Hour N, where N = selected horizon (1–6).
* Y-axis: P2 … P9.
* The configurable dots are draggable vertically and snap to an integer
* P-level. The line connects hour 0 through the horizon limit.
*/
function PLevelConfigChart({
 ...
 const hoverClearTimerRef = useRef(/** @type {ReturnType<typeof setTimeout> | null} */ (null));
 const popupHoveredRef = useRef(false);
 const [hoveredHour, setHoveredHour] = useState(/** @type {number | null} */ (null));
 const [evalBtnHovered, setEvalBtnHovered] = useState(false);


 // Horizon limits visible hours (0..N) and tints the configurable forecast region.
 ...
 const hoverDetail = useMemo(() => {
   if (hoveredHour == null || hoveredHour < 0 || hoveredHour > horizon) return null;
   const pLevel = yValues[hoveredHour];
   return {
     hour: hoveredHour,
     hourLabel: hoveredHour === 0 ? 'Current Hour' : `Hour ${hoveredHour}`,
     pLevel,
     evalEnabled: hoveredHour === 0 || hasResult,
   };
 }, [hoveredHour, yValues, hasResult, horizon]);


 const scheduleClearHover = useCallback(() => {
   if (hoverClearTimerRef.current) clearTimeout(hoverClearTimerRef.current);
   hoverClearTimerRef.current = setTimeout(() => {
     if (!popupHoveredRef.current) setHoveredHour(null);
   }, 80);
 }, []);
 ...
 const options = useMemo(
   ...
   [yValues, gridColor, subtext, horizon, scheme.primary],
 );


 const optionsWithHover = useMemo(
   () => ({
     ...options,
     onHover: (_event, elements) => {
       ...
       if (elements?.length) {
         ...
         if (idx >= 0 && idx <= horizon) setHoveredHour(idx);
       } else {
         scheduleClearHover();
       }
     },
   }),
   [options, cancelClearHover, scheduleClearHover, horizon],
 );
 ...
 const updatePinAnchor = useCallback(() => {
   ...
   const pt = meta?.data?.[hoveredHour];
   if (!pt || pt.skip || !Number.isFinite(pt.x) || !Number.isFinite(pt.y)) {
     setAnchorVP(null);
     return;
   }
 ...
 useLayoutEffect(() => {
   ...
   const ro = canvas ? new ResizeObserver(() => updatePinAnchor()) : null;
 ...
 const evalPopup = hoverDetail && anchorVP && typeof document !== 'undefined'
   ? createPortal(
       <div
         ref={overlayRef}
         onMouseEnter={() => {
         ...
         onMouseLeave={() => {
           ...
           setEvalBtnHovered(false);
           setHoveredHour(null);
         }}
         onClick={e => e.stopPropagation()}
         role="tooltip"
         aria-label="P-level hour details"
         style={{
           position: 'fixed',
           left: anchorVP.left,
           top: anchorVP.top,
         ...
         <div className="mb-1">
           <span style={{ color: subtext }} className="font-semibold">{hoverDetail.hourLabel}</span>
         </div>
         <div className="flex items-baseline gap-1.5 mb-2">
           <span style={{ color: subtext }} className="text-[10px]">P-level:</span>
           <span style={{ color: scheme.primary }} className="font-mono font-semibold text-sm">
             P{hoverDetail.pLevel}
           </span>
         </div>
         <div
           className="relative"
           onMouseEnter={() => {
             if (!hoverDetail.evalEnabled) setEvalBtnHovered(true);
           }}
           onMouseLeave={() => setEvalBtnHovered(false)}>
           <button
             type="button"
             onClick={() => {
               if (!hoverDetail.evalEnabled) return;
               navigate(`/policy?hour=${hoverDetail.hour}`);
             }}
             disabled={!hoverDetail.evalEnabled}
             title={
               hoverDetail.evalEnabled
                 ? `Open Policy Evaluation at ${hoverDetail.hourLabel}`
                 : undefined
             }
             style={{
               background: !hoverDetail.evalEnabled ? border : scheme.primary,
               color: !hoverDetail.evalEnabled ? subtext : 'white',
               opacity: !hoverDetail.evalEnabled ? 0.6 : 1,
               cursor: !hoverDetail.evalEnabled ? 'not-allowed' : 'pointer',
             }}
             className="w-full px-2 py-1.5 rounded-md text-[11px] font-semibold">
             Evaluate
           </button>
           {evalBtnHovered && !hoverDetail.evalEnabled && (
             <p
               style={{ color: subtext, background: card, borderColor: border }}
               className="absolute left-1/2 -translate-x-1/2 bottom-full mb-1.5 w-max max-w-[200px] px-2 py-1 rounded-md border text-[10px] font-medium text-center shadow-sm pointer-events-none z-[10001]">
               Run the forecast first!
             </p>
           )}
         </div>
       </div>,
 ...
 useEffect(() => {
   setEvalBtnHovered(false);
   setHoveredHour(null);
 }, [currentLevel, hasResult, horizon]);
 ...
 useEffect(() => {
   ...
   // chartIdx maps to pumpSequence[chartIdx - 1] for hours 1..horizon.
   // chartIdx 0 (current state) is read-only.
 ...
 return (
   <div
     ...
     onMouseLeave={() => {
       if (!dragRef.current) scheduleClearHover();
     }}>
     <Line ref={chartRef} data={data} options={optionsWithHover} />
     {evalPopup}
   </div>
 );
}
             ...
             <div className="flex-1 min-h-0 flex">
               <div className="flex-1 min-w-0 flex flex-col">
                 <div style={{ color: subtext }} className="text-xs font-semibold text-center mb-2">
                   Hover over the dots to evaluate simulated P-level!
                 </div>
